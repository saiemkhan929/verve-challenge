...
1. **Use of Redis**:
    - To handle deduplication across multiple instances, I introduced **Redis** as a distributed in-memory data store.
...
- **Distributed Deduplication**: By using Redis, all instances of the application share the same state, ensuring that deduplication is handled across all instances.
- **Scalability**: Redis is a scalable, high-performance distributed data store, making it ideal for ensuring consistency in distributed systems.
...
1. **Kafka as a Streaming Service**:
    ...
    - A new **KafkaProducerService** was created to send the count of unique requests to a Kafka topic named `unique-id-counts`.
...
- **Scalability**: The use of Redis and Kafka ensures that the application can scale horizontally across multiple instances and process high volumes of requests.
- **Concurrency**: The application is designed to handle concurrent requests efficiently using thread-safe collections and asynchronous processing.
- **Resilience**: Both Redis and Kafka provide fault-tolerant, distributed architectures, ensuring that the system remains resilient even under heavy load.
